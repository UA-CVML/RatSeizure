...
class LocalMaskedMHCA_TopK(nn.Module):
    def __init__(
        self,
        n_embd,
        n_head,
    ...
    def forward(self, x, mask):
        ...
        Tq = q.size(-1)
        Tk = k.size(-1)
        q = q.view(B, self.n_head, self.n_channels, Tq).transpose(2, 3)
        k = k.view(B, self.n_head, self.n_channels, Tk).transpose(2, 3)
        v = v.view(B, self.n_head, self.n_channels, Tk).transpose(2, 3)

        # build window indices for each query position
        k_pad = F.pad(k, (0, 0, w, w), value=0.0)
        v_pad = F.pad(v, (0, 0, w, w), value=0.0)
        m_pad = F.pad(kv_mask, (w, w), value=False)

        device = x.device
        offsets = torch.arange(-w, w + 1, device=device)
        centers = torch.arange(Tq, device=device).unsqueeze(1) + w
        idx = centers + offsets.unsqueeze(0)
        idx = idx.clamp(0, Tk + 2 * w - 1).long()

        # gather windowed K/V and masks
        idx_kv = idx.view(1, 1, Tq, W, 1).expand(B, self.n_head, Tq, W, self.n_channels)
        k_win = torch.take_along_dim(k_pad.unsqueeze(3), idx_kv, dim=2)
        v_win = torch.take_along_dim(v_pad.unsqueeze(3), idx_kv, dim=2)

        idx_m = idx.view(1, 1, Tq, W).expand(B, 1, Tq, W)
        m_win = torch.take_along_dim(m_pad.unsqueeze(2), idx_m, dim=3)

        score = (q.unsqueeze(3) * k_win).sum(-1)
        m_win_h = m_win.expand(B, self.n_head, Tq, W)
        score = score.masked_fill(~m_win_h, -1e4)
        if self.keep_ratio < 1.0:
            M = max(1, int(math.ceil(W * self.keep_ratio)))
            center_pos = w
            if M == 1:
                topk = torch.full((B, self.n_head, Tq, 1),center_pos,device=score.device,dtype=torch.long)
            else:
                score_wo_center = score.clone()
                score_wo_center[..., center_pos] = -1e4
                score_wo_center = score_wo_center.masked_fill(~m_win_h, -1e9)
                topk_other = torch.topk(score_wo_center, k=M-1, dim=-1, largest=True, sorted=False).indices

                center_idx = torch.full((B, self.n_head, Tq, 1),center_pos,device=score.device,dtype=topk_other.dtype)
                topk = torch.cat([center_idx, topk_other], dim=-1)

            topk_kv = topk.unsqueeze(-1).expand(B, self.n_head, Tq, M, self.n_channels)
            k_sel = torch.gather(k_win, dim=3, index=topk_kv)
            v_sel = torch.gather(v_win, dim=3, index=topk_kv)
            m_sel = torch.gather(m_win_h, dim=3, index=topk)

            # relative position (gather the same selected positions)
            if self.use_rel_pe:
                rel = self.rel_pe.squeeze(0).squeeze(0)
                rel = rel.unsqueeze(0).unsqueeze(2).expand(B, self.n_head, Tq, W)
                rel_sel = torch.gather(rel, dim=3, index=topk)
        else:
            M = W
            k_sel, v_sel = k_win, v_win
            m_sel = m_win.expand(B, self.n_head, Tq, W)
            if self.use_rel_pe:
                rel_sel = self.rel_pe.squeeze(0).squeeze(0).unsqueeze(0).unsqueeze(2).expand(B, self.n_head, Tq, W)

        # attention on selected tokens
        q_scaled = q * self.scale
        att = (q_scaled.unsqueeze(3) * k_sel).sum(-1)

        if self.use_rel_pe:
            att = att + rel_sel

        att = att.masked_fill(~m_sel, -1e4)
        att = F.softmax(att, dim=-1)
        att = self.attn_drop(att)

        out = (att.unsqueeze(-1) * v_sel).sum(dim=3)
        out = out.transpose(2, 3).contiguous().view(B, C, Tq)
        out = self.proj_drop(self.proj(out)) * qx_mask.to(out.dtype)
        return out, qx_mask
    ...
    def __init__(
        ...
        if mha_win_size > 1:
            if attn_keep_ratio < 1.0:
                self.attn = LocalMaskedMHCA_TopK(
                    n_embd, n_head,
