...
def valid_one_epoch(
    ...
    model.eval()
    # timing stats
    per_video_times = []   # list of floats (sec/video)
    per_video_ids = []     # aligned list of video_id (optional)
    ...
    for iter_idx, video_list in enumerate(val_loader, 0):
        ...
        with torch.no_grad():
            ...
            num_vids = len(output)  # or len(video_list)
            sec_per_vid = elapsed / max(num_vids, 1)
            for vid_idx in range(num_vids):
                per_video_times.append(sec_per_vid)
                per_video_ids.append(output[vid_idx]['video_id'])
    ...
    # ----- overall avg time/video -----
    avg_sec_per_vid = float(np.mean(per_video_times)) if per_video_times else 0.0
    print(f"[Timing] Avg forward time: {avg_sec_per_vid:.6f} sec/video ({1.0/avg_sec_per_vid:.2f} videos/sec)" if avg_sec_per_vid > 0 else "[Timing] No timing samples collected.")
